...
<body>
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<ul class="nav nav-pills">
						<li role="presentation" class="active"><a href="../login/login_index.php">登入首頁</a></li>
					</ul>
				</div>
			</div>
		</div>
				...
				<div class="col-xs-12">
					<form class="form-horizontal" id="register_form">
						...
						<div class="form-group">
							...
							<div class="col-sm-2">
								<input type="number" class="form-control" id="S_Number" name="S_Number" min="1" max="20000" placeholder="幾" required>
							</div>
							<label for="S_Number" class="col-sm-1 control-label">號</label>
							<label class="col-sm-1 control-label">到</label>
							<div class="col-sm-2">
								<input type="number" class="form-control" id="E_Number" name="E_Number" min="1" max="20000" placeholder="幾" required>
							</div>
						...
						<div class="form-group">
							<div class="col-sm-offset-2 col-sm-10">
								...
								<a class="btn btn-default" href="../login/login_index.php" role="button">取消</a>
							</div>
						</div>
					</form>
	...
	<script src="https://maps.googleapis.com/maps/api/js?key=秘密callback=initMap" async defer></script>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#address").on("blur", function() {
				//取得輸入的值
				var adrr = $("#address").val();
				var nums = $("#S_Number").val();
				var nume = $("#E_Number").val();
				//console.log(adrr);
				//console.log(nums);
				//console.log(nume);
				//當keyup的時候，裡面的值不是空字串的話，就檢查。
				...
				if ($("#password").val() != $("#confirm_password").val()) {
					...
					$.ajax({
						...
						if (data == "yes") {
							...
							//註冊新增成功，轉跳到登入頁面。
							window.location.href = "../login/login_god.php";
						} else {
							alert("註冊失敗，請與系統人員聯繫");
						}
					}).fail(function(jqXHR, textStatus, errorThrown) {
