...
$family_data = get_edit_family($_GET['i']); //編輯聯絡人
//print_r($family_data);
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-xs-12">
                    <form id="family">
                        <input type="hidden" id="id" value="<?php echo $family_data['id'] ?>">
                        <div class="form-group">
                            <label for="InputName">名稱</label>
                            <input type="text" class="form-control" id="InputName" value="<?php echo $family_data['name']; ?>" required>

                        </div>
                        <div class="form-group">
                            <label for="category">分類</label>
                            <select id="category" class="form-control">
                                <option value="手機" <?php echo ($family_data['category'] == "手機") ? 'selected' : ''; ?>>手機</option>
                                <option value="市話" <?php echo ($family_data['category'] == "市話") ? 'selected' : ''; ?>>市話</option>
                                <option value="網頁" <?php echo ($family_data['category'] == "網頁") ? 'selected' : ''; ?>>網頁</option>
                            </select>
                        </div>
                        <div class="form-group" id="InputLink_form">
                            <?php if ($family_data['category'] == "手機") : ?>
                                <input type="tel" pattern="[0-9]{4}[0-9]{3}[0-9]{3}" class="form-control" id="InputLink" name="InputLink" placeholder='0912345678' value="<?php echo $family_data['link']; ?>" required>
                            <?php elseif ($family_data['category'] == "市話") : ?>
                                <input type="tel" pattern="[0-9]{2}[0-9]{3,4}[0-9]{4}" class="form-control" id="InputLink" name="InputLink" placeholder='04123(4)5678' value="<?php echo $family_data['link']; ?>" required>
                            <?php else : ?>
                                <input type="url" class="form-control" id="InputLink" name="InputLink" placeholder='http://www.ee.nchu.edu.tw/' value="<?php echo $family_data['link']; ?>" required>
                            <?php endif; ?>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#InputName").on("keyup", function() {
                ...
                if (keyin_value != '' && keyin_value != '<?php echo $family_data['name']; ?>') {
                    //$.ajax 是 jQuery 的方法，裡面使用的是物件。
                    $.ajax({
                        type: "POST", //表單傳送的方式 同 form 的 method 屬性
                        url: "../php_pe/check_linkname.php", //目標給哪個檔案 同 form 的 action 屬性
                        ...
                        if (data == "yes") {
                            ...
                            $("form#family button[type='submit']").addClass('disabled');
                            $("form#family button[type='submit']").attr('disabled', true);
                        }
            ...
            //當表單 sumbit 出去的時候
            $("form#family").on("submit", function() {
                if ($("#InputName").val() == '' || $("#InputLink").val() == '') {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../php_pe/update_family.php", //因為此 login.php 是放在 admin 資料夾內，若要前往 php，就要回上一層 ../ 找到 php 才能進入 verify_user.php
                        data: {
                            id: $("#id").val(), //id
                            name: $("#InputName").val(), //聯絡人名稱
                            cate: $("#category").val(),
                            link: $("#InputLink").val() //聯絡人link
                        },
                        dataType: 'html' //設定該網頁回應的會是 html 格式
                    }).done(function(data) {
                        //成功的時候
