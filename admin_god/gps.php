    ...
    function tevt(diss) {
        if ($('#distance').value != 0) {
            ...
            $.ajax({
                ...
                url: "../php_god/gps_check.php", //目標給哪個檔案 同 form 的 action 屬性
                data: { //為要傳過去的資料，使用物件方式呈現，因為變數key值為英文的關係，所以用物件方式送。ex: {name : "輸入的名字", password : "輸入的密碼"}
                    n: diss //代表要傳一個 n 變數值為，username 文字方塊裡的值
                },
                dataType: 'html' //設定該網頁回應的會是 html 格式
            }).done(function(data) {
                if (data == "yes") {
                    alert("在安全距離以內,將前往資訊頁面");
                    window.location.href = "https://cs.ee.nchu.edu.tw<?php echo dirname($_SERVER['PHP_SELF']); ?>/qrnumber.php?id=<?php echo base64_encode($datas['id']); ?>";
                } else {
                    //console.log(data);
                    alert("太遠了！");
